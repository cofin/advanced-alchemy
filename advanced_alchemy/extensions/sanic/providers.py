...
if TYPE_CHECKING:
    ...
    from advanced_alchemy.extensions.sanic.config import SQLAlchemyAsyncConfig
...
async def provide_async_services(
    *inputs: "SQLAlchemyAsyncServiceCompositionInput[Any]",
    ...
    """Compose and open async services in one call — Sanic-aware.
    ...
    2.  **Request-scoped session**: If ``request`` is provided, the session is
        resolved from ``request.ctx.<session_key>``. This assumes the
    ...
    Args:
        *inputs: Service classes or provider generators.
        ...
        request: An optional Sanic request to resolve the session from.
    ...
    if session is not None:
        ...
        sess = getattr(request.ctx, config.session_key, None)
        if sess is None:
            msg = f"No session in request.ctx.{config.session_key} — ensure the extension's middleware is registered"
